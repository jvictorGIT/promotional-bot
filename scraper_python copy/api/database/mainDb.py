...
def salvar_no_banco(dados_item):
    ...
    try:
        ...
        if existe > 0:                                                                                                                                                                              
            ...
            cursor.execute("""
                UPDATE Promocoes
                SET Name = ?, PriceDe = ?, PricePor = ?, Link = ?, ImageUrl = ?, Status = 'Pending', Percentual = 0
                WHERE Sku = ?
            """, (dados_item['nome'], preco_de, preco_por, dados_item['link'], dados_item.get('img'), dados_item['SKU']))
        ...
        sql_insert = """
            INSERT INTO Promocoes (Sku, Name, PriceDe, PricePor, Link, ImageUrl, Status)
            VALUES (?, ?, ?, ?, ?, ?, ?)
        """
